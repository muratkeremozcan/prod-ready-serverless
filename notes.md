...
>  Note:
> Diffing does not work in CI before there is something on main. Maybe we need to merge first and then add diffing.
>
> ```yml
>       - name: update the schema with Optic
>         run: |
>           npm run optic:update
> 
>       # - name: diff the schema with Optic
>       #   run: |
>       #     npm run optic:diff
> 
